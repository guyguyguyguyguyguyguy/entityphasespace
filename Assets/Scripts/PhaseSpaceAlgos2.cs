...
namespace PhaseSpcaeAlgorithms
{
    ...
    public static class ChansAlgorithm
    {
        ...
        private static Vector3[] ChunkConvexHull(Vector3[][] chunks)
        {
            ...
            using(System.IO.StreamWriter file = new System.IO.StreamWriter("graham_scan.csv"))
            {
        ...
        {
            Vector3[][] chunks = ChunkPoints(points, m);

            Vector3[] chunkedPossiblePoints = ChunkConvexHull(chunks);

            Vector3[] leftOverPoints = (Vector3[]) chunkedPossiblePoints.Clone();
            List<Vector3> convexHull = new List<Vector3>();
            Vector3 prevPoint = Vector3.zero;
            prevPoint.x = int.MinValue;

            Vector3 currPoint = ExtensionMethods.GetLeftMost(chunkedPossiblePoints);
    ...
    static class AltJarvisAlgorithm
    {
        public static Vector3 JarvisMarch(Vector3 point , Vector3[] points, Vector3 prevPoint)
        {
            ...
            Func<Vector3, Vector3, double> euclDistNoSqrt = (a, b) => (a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y);
